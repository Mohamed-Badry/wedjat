...
TARGET_NORAD_ID = int(os.getenv("TARGET_NORAD_ID", 43880))
...
def main():
    ...
    demodulator_cls = DemodulatorRegistry.get(TARGET_NORAD_ID)
    if not demodulator_cls:
        logger.error(f"No demodulator registered for NORAD ID: {TARGET_NORAD_ID}")
        return
    ...
    def on_frame_decoded(frame_bytes: bytes):
        ...
        frame_count += 1
        payload = {
            "norad_id": TARGET_NORAD_ID,
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "raw_frame": frame_bytes.hex(),
            "station_id": STATION_ID,
            "snr": 15.0,  # Default SNR
        }
        topic = f"telemetry/live/{TARGET_NORAD_ID}"
    ...
    logger.info(f"Starting live stream for NORAD {TARGET_NORAD_ID} listening on {ZMQ_ENDPOINT}")
    
    try:
        demodulator.start_live_stream(ZMQ_ENDPOINT, on_frame_decoded)
        # Keep the main thread alive while GNU Radio processes asynchronously
        logger.info("Entering main loop...")
        loop_count = 0
        while True:
            time.sleep(1)
            loop_count += 1
            logger.info(f"Main loop tick {loop_count}")
